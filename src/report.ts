...
function countInteresting(line: string): {
	err: number;
	fail: number;
	xfail: number;
} {
...
function curateJobExcerpt(raw: string): {
	excerpt: string;
...
export function buildMarkdownXmlReport(
	...
	let commentsSectionChars = 0;
	{
		...
		sectionStart(
			"## PR Comments (since last push)",
			`<pr-comments since="${sinceIso}">`,
		);
	...
	if (!flags.claudeOnly) {
		const preLen = lines.join("\n").length;
		sectionStart(
			"## Failing CI (runs since last push)",
			`<ci-runs branch="${branch}" sha="${sha.slice(0, 7)}">`,
		);
		if (!runExtracts.length) {
			...
			for (const rx of runExtracts) {
				const r = rx.run;
				lines.push(
					`### Run #${r.id}${r.name ? ` — ${r.name}` : ""} — ${r.status}/${r.conclusion || ""}`,
				);
				lines.push(`${r.url}`);
				lines.push(`<ci-run id="${r.id}">`);
				lines.push(
					`<run-meta createdAt="${r.createdAt || ""}" headSha="${(r.headSha || "").slice(0, 7)}"/>`,
				);
	...
	sectionStart(
		"## Summary of failing CI (Claude Code)",
		`<ci-summary engine="${claudeSummary ? "claude" : "none"}">`,
	);
	if (claudeSummary) {
